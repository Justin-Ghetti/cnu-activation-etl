...
Each week, a legacy reporting tool emailed a report to the team manually. From there:
...
- Multiply across a team of 8, and this was hours of repetitive manual work every single week
...
2. **Transforms** it using business logic to categorize every activation automatically
...
> Note: all schema, table, and field names below are illustrative placeholders. Real internal names have been replaced to keep proprietary system details private while still showing the data model.

All data lives in a Snowflake data warehouse drawing from two source systems:

**Entitlement Data Warehouse**
- `ACTIVATIONS.ACTIVATION_RECORDS` — raw machine activation records
- `ACTIVATIONS.ACTIVATION_EVENTS` — event details (who activated, when, event type)
- `DIMENSIONS.ENTITLEMENT_DIM` — license and product information
- `DIMENSIONS.ACCOUNT_DIM` — account and company information
- `DIMENSIONS.PRODUCT_DIM` — product names

**CRM Mirror (Salesforce data replicated into Snowflake)**
- `CRM_MIRROR.AGREEMENT_ATTRIBUTES` — master license contracts
- `CRM_MIRROR.AGREEMENT_TERMS` — license pricing model and policy type
- `CRM_MIRROR.ENGAGEMENT_OWNER` — engagement owner assignment
...
### 1. Date Window
Automatically resolves the current reporting week (Sunday to Saturday) without any manual input. Mirrors the behavior of the legacy reporting tool.

### 2. Valid License Filter
Enforces a policy allow-list, including only licenses under the eligible pricing models. Automatically excludes contract types that fall outside the reporting scope.

### 3. Current Week Events
Pulls all CNU (Concurrent + Counted Named User) activation events for the reporting window. Filters out internal accounts. Tags each product as Desktop or Server by base product code.
...
- Did a Desktop activation occur this week (the "anchor" signal)?

### 8. Owner Lookup
Identifies the most recent engagement owner for each account, including closed engagements, so every activation has an assigned owner.
...
|---|---|---|
| 0 | **New Desktop Product Activated** | Desktop anchor exists this week, bundles all Desktop + Server on that host |
| 1 | **New Desktop Product Activation on Non-Desktop Product Server** | Desktop on a server-history host, no Desktop anchor ever, first-time active |
| 2 | **Enterprise Server Product Activated on Unique Server** | First-time Server host, no Desktop anchor |
...
- Eliminated ~80+ hours of manual work per year across the team
- Removed the risk of human categorization errors
- Cases are created and emails sent automatically, with no rep action required to initiate customer outreach
...
This repository is a sanitized portfolio version. All schema names, table names, field names, and account identifiers have been replaced with generic placeholders, and any sample data is illustrative and does not reflect real systems or customer information.
